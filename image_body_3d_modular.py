...
class Config:
    ...
    SOURCES = {
        'stereo': {'params': 'stereo/stereo_params_sidebyside.npz', 'image_prefix': 'stereo/test_ss', 'output_base': 'stereo'},
...
def main():
    ...
    # Configure paths based on source
    if args.source in Config.SOURCES:
        sc = Config.SOURCES[args.source]
        Config.PARAMS_FILE, Config.IMAGE_PREFIX, Config.OUTPUT_BASE = sc['params'], sc['image_prefix'], sc['output_base']
        print(f"\n📷 {args.source} camera → {Config.OUTPUT_BASE}/")
    ...
    output = OutputManager(os.path.join(Config.OUTPUT_BASE, Config.OUTPUT_DIRS[args.model]), args.model)
    
    # Load images
    suffix = args.num if args.num else ''
    img_left_path = f"{Config.IMAGE_PREFIX}_left{suffix}.png"
    img_right_path = f"{Config.IMAGE_PREFIX}_right{suffix}.png"
