...
DEBUG VERSION - 3D Biomechanical Analysis with Intermediate Outputs
...
class Config:
    ...
    # Visualization
    MIN_AXIS_LIMIT = 600  # mm
    
    # Output directories
    OUTPUT_DIRS = {
        'mediapipe': 'biomech_analysis_mediapipe',
        'hrnet': 'biomech_analysis_hrnet'
    }
...
class MediaPipeModel(PoseModel):
    ...
    def detect(self, frameL, frameR):
        ...
        debug_dir = 'debug_intermediate'
        os.makedirs(debug_dir, exist_ok=True)
        
        debug_img_left = frameL.copy()
        for name, pt in kp_left.items():
            x, y = int(pt[0]), int(pt[1])
            cv2.circle(debug_img_left, (x, y), 5, (0, 255, 0), -1)
            cv2.putText(debug_img_left, name, (x+5, y-5), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1)
        
        cv2.imwrite(f'{debug_dir}/01_mediapipe_2d_keypoints.png', debug_img_left)
        print(f"[DEBUG] Saved 2D keypoints → {debug_dir}/01_mediapipe_2d_keypoints.png")
        print(f"[DEBUG] Detected {len(kp_left)} keypoints")
...
def main():
    ...
    # Debug mode is ALWAYS ON in this version
    ...
    output = OutputManager(Config.OUTPUT_DIRS[args.model], args.model)
    ...
    os.makedirs(debug_dir, exist_ok=True)
    print(f"\n{'='*70}")
    print(f"DEBUG MODE: Saving intermediate outputs to {debug_dir}/")
    print(f"{'='*70}\n")
    ...
    if kp_left is None or kp_right is None:
        print("Error: Pose detection failed")
        return
    
    # Reconstruct 3D
    print(f"\n[STAGE 2] 3D Reconstruction...")
    points = pose3d.reconstruct(kp_left, kp_right, verbose=args.verbose)
    ...
    # DEBUG: Save measurements
    with open(f'{debug_dir}/03_measurements.txt', 'w') as f:
        f.write("="*70 + "\n")
        f.write("BIOMECHANICAL MEASUREMENTS\n")
        f.write("="*70 + "\n\n")
        
        f.write("SEGMENT LENGTHS (cm):\n")
        f.write("-"*40 + "\n")
        for name, length in sorted(segments.items()):
            f.write(f"{name:<25} {length:8.2f} cm\n")
        
        f.write("\nJOINT ANGLES (degrees):\n")
        f.write("-"*40 + "\n")
        for name, angle in sorted(angles.items()):
            f.write(f"{name:<25} {angle:8.1f}°\n")
        
        f.write(f"\nESTIMATED HEIGHT: {measured_height:.2f} cm\n")
    
    print(f"[DEBUG] Saved measurements: {debug_dir}/03_measurements.txt")
    print(f"[DEBUG] Calculated height: {measured_height:.2f} cm")
