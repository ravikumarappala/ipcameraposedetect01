...
def copy_files(src, dest, num):
    ...
    # Detect image naming pattern by checking existing files
    if src == 'stereo':
        # stereo uses: test_image1.png format (no left/right)
        ...
        # ip uses: test_ss_left1.png, test_ss_right1.png
        for s in ['left','right']:
...
def main():
    ...
    print('\n📋 Copying...')
    Config.PARAMS_FILE, img_pre = copy_files(args.source, session, args.num)
    ...
    # stereo: combined side-by-side, ip: separate left/right
    if args.source == 'stereo':
        combined_path = session + '/' + img_pre + suffix + '.png'
        combined_img = cv2.imread(combined_path)
        if combined_img is None:
            print('Error: Could not read: ' + combined_path)
            return
        frameL, frameR = split_sidebyside_image(combined_img)
        print('✓ Split side-by-side image')
    else:
        img_left_path = session + '/' + img_pre + '_left' + suffix + '.png'
        img_right_path = session + '/' + img_pre + '_right' + suffix + '.png'
